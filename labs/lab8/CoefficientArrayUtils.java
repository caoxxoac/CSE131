...
public class CoefficientArrayUtils {
	...
	public static double[] createNextCoefficients(double coefficient, int degree, double[] prevCoefficients) {
		int prevLength = prevCoefficients.length;
		double[] newC;
		if(degree < prevLength) {
			newC = new double[prevLength];
			for(int i=0; i<prevLength; i++) {
				if (degree == i) {
					newC[i] = prevCoefficients[i] + coefficient;
				}
				else {
					newC[i] = prevCoefficients[i];
				}
			}
		}
		else {
			newC = new double[degree+1];
