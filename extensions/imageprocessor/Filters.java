...
public class Filters {
	...
	public static Color purplish(Color c) {
		...
		int green = 0;
		int blue = c.getBlue();
		Color colorArray = new Color(red, green, blue);
		return colorArray;  
	}
	...
	public static Color grayscale(Color c) {
		...
		return new Color(color, color, color);  // FIXME
	}
	...
	public static Color blackAndWhite(Color c) {
		...
		int green = c.getGreen();
		int blue = c.getBlue();
		int color = (red + green + blue) / 3 / 128;
		if (color == 0) {
			return Color.black;
		}
		else {
	...
	public static Color bgSubtract(Color source1Color, Color source2Color, int tolerance) {
		if (Math.abs(source1Color.getGreen() - source2Color.getGreen()) <= tolerance ||
				...
				Math.abs(source1Color.getRed() - source2Color.getRed()) <= tolerance) {
			return Color.blue;
		}
		else {
			return source1Color;
	...
	public static Color bgReplace(Color s1Color, Color s2Color) {
		if (s1Color.getGreen() == Color.blue.getGreen() &&
				s1Color.getBlue() == Color.blue.getBlue() &&
				s1Color.getRed() == Color.blue.getRed()) {
			return s2Color;
		}
		else {
			return s1Color;
